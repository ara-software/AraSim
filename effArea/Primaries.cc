...
//    }
	...
	//Interaction::PickNear (antarctica, detector, settings1);
        L0 = PickNear (antarctica, detector, settings1);
...
//-------------------------------------------------- 
    ...
    if (settings1->GETCHORD_MODE==0 && settings1->SIM_GEOMETRY==0) {  // pick_near && Veff
    //if (settings1->GETCHORD_MODE==0) {  // pick_near && Veff
    ...
    else if (settings1->GETCHORD_MODE==1 || settings1->SIM_GEOMETRY==1) {
    ...
    cout << "1: weight: " << weight << " prob: " << probability << endl;
...
double Interaction::PickNear (IceModel *antarctica, Detector *detector, Settings *settings1) {
  ...
  const double range = settings1->POSNU_RADIUS;   // test value, 2km radius. can be changed to read from Settings
    ...
    if(settings1->SIM_GEOMETRY==0) { // if cylindlical geometry setup for Veff (default)
    ...
    else if(settings1->SIM_GEOMETRY==1) { // if spherical geometry setup for Aff
