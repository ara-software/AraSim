...
// end of values from icemc
  ...
  NNU_D_THETA=0.0873;       // default : nnu d_theta : 5 deg
    ...
    SIM_GEOMETRY=0; // Simulation geometry. 0: Cylindrical coordinate for Veff (default), 1: Spherical coordinate for Aeff
}
  ...
  if ( setFile.is_open() ) {
      while (setFile.good() ) {
          ...
          if (line[0] != "/"[0]) {
              ...
              else if (label == "SIM_GEOMETRY") {
                  SIM_GEOMETRY = atoi( line.substr(line.find_first_of("=") + 1).c_str() );
              }
          }
      }
      setFile.close();
