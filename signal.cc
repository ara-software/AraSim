...
void Signal::Initialize(){
  ...
  x0ice=0.403; // radiation length of ice (meters)
...
void Signal::GetVmMHz(double vmmhz_max,double vmmhz1m_max,double pnu,double *freq,double notch_min,double notch_max,double *vmmhz,int nfreq) {
  ...
  for (int i=0;i<nfreq;i++) {
    vmmhz[i]=vmmhz_max *GetVmMHz1m(pnu,freq[i])/vmmhz1m_max;
...
void Signal::GetSpread(double pnu, double emfrac, double hadfrac, double freq, double& deltheta_em_max, double& deltheta_had_max) {
...
void Signal::TaperVmMHz(double viewangle, double deltheta_em, double deltheta_had, double emfrac, double hadfrac, double& vmmhz1m, double& vmmhz1m_em_obs) {
  ...
  { // if the em component is essentially zero than set this to zero
      vmmhz1m_em=0;
  }
  ...
  if (hadfrac!=0) { // if there is a hadronic fraction
    if (calpulser == false){
      ...
      if (rtemp<20) { // if we are less than 20 sigma from cerenkov angle
        ...
        // if we're more than 20 sigma from cerenkov angle
...
void Signal::AskaryanModule( Event *event, Settings *settings1, double viewangle, double atten_factor, int outbin, double *Tarray, double *Earray, int &skip_bins )
{

  //we use the Hanson-Connolly model (https://arxiv.org/abs/1605.04975)
  //code base lives here: https://github.com/918particle/AskaryanModule
  //good tutorial here: https://github.com/918particle/AskaryanModule/tree/simplified/tutorial
  ...
  if ( settings1->SHOWER_MODE == 0 || settings1->SHOWER_MODE == 1 || settings1->SHOWER_MODE == 3 ) { // only EM or only HAD
    ...
    //first, let's set up some time and frequency arrays
    //in the case where we're running just one shower or another, we can use the same time/frequency arrays

    // ok, let's just calculate near signal time bins
    int shower_bin = event->Nu_Interaction[0].shower_Q_profile.size();
    ...
    float dt = Tarray[1] - Tarray[0]; //ns
    float fc = 1.0/(2.0 * dt); //GHz
    float df = 2.0 * fc / float(outbin); //Divide the bandwidth into correct number of bins
    for(float x=0; x<fc; x=x+df){
      freqs->push_back(x);
    }
    h->setAskFreq(freqs);


    //we must specify EM or HAD shower
    ...
    if ( event->Nu_Interaction[0].EM_LQ > 0 ) {
      ...
      int shower_bin = event->Nu_Interaction[0].EM_shower_Q_profile.size();
      // ok, let's just calculate near signal time bins
      double shower_dt = (settings1->SHOWER_STEP * shower_bin) / c_ns; // shower time length in ns
      double test_signal_window = fabs(offcone_factor) * shower_dt * 1.2 + 2.; // additional 2 ns for changle_ice case, 20% additional time
      // we also have to calculate when the window should start
    ...
    if ( event->Nu_Interaction[0].HAD_LQ > 0 ) {
      ...
      if(EM_shower_on==0){ //if there was no EM shower, we have to set up the T array parts anew
        
        int shower_bin = event->Nu_Interaction[0].HAD_shower_Q_profile.size();
        // ok, let's just calculate near signal time bins
        double shower_dt = (settings1->SHOWER_STEP * shower_bin) / c_ns; // shower time length in ns
        double test_signal_window = fabs(offcone_factor) * shower_dt * 1.2 + 2.; // additional 2 ns for changle_ice case, 20% additional time
        // we also have to calculate when the window should start
    ...
    if(EM_shower_on || HAD_shower_on){ //only try if you have a had or an EM shower
      //now that we have the time domain information set up
      //we can do the frequency bits
      vector<float> *freqs = new vector<float>;
      float dt = Tarray[1] - Tarray[0]; //ns
      float fc = 1.0/(2.0 * dt); //GHz
      float df = 2.0*fc/float(outbin);
      for(float x=0; x<fc; x=x+df) freqs->push_back(x);

      //now to get the Askaryan bits for the EM shower
      if(EM_shower_on){
        Askaryan *h = new Askaryan();
        h->setFormScale(1/(sqrt(2.0*3.14159)*0.051)); //The 0.051 is in meters, so the 1/e width is 5.1 cm.
        h->setAskFreq(freqs);
      ...
      //now to get the Askaryan bits for the HAD shower
      if(HAD_shower_on){
        Askaryan *h = new Askaryan();
        h->setFormScale(1/(sqrt(2.0*3.14159)*0.051)); //The 0.051 is in meters, so the 1/e width is 5.1 cm.
        h->setAskFreq(freqs);
...
void Signal::EugeneModule( Event *event, Settings *settings1, double viewangle, double atten_factor, int outbin, double *Tarray, double *Earray, int &skip_bins )
{

  //Eugene Hong's custom time domain model
  //best documentation is found in his thesis: http://rave.ohiolink.edu/etdc/view?acc_num=osu1417624919
  ...
  if ( settings1->SHOWER_MODE == 0 || settings1->SHOWER_MODE == 1 || settings1->SHOWER_MODE == 3 ) { // only EM or only HAD
    ...
    for (int tbin=0; tbin<outbin; tbin++)
    {
      ...
      for (int bin=0; bin<new_shower_bin-2; bin++)
      {
        ...
        if ( event->Nu_Interaction[0].shower_Q_profile[mid_old_bin]<=0) Integrate += 0.;
        else Integrate += -1.*(event->Nu_Interaction[0].shower_Q_profile[mid_old_bin]) * Param_RE_Tterm_approx(Tterm, param_RA);
      }
    ...
    if ( event->Nu_Interaction[0].EM_LQ > 0 ) {
      ...
      for (int tbin=0; tbin<outbin; tbin++)
      {
        ...
        for (int bin=0; bin<new_shower_bin-2; bin++) {
          ...
          if ( event->Nu_Interaction[0].EM_shower_Q_profile[mid_old_bin]<=0) Integrate += 0.;
          else  Integrate += -1.*(event->Nu_Interaction[0].EM_shower_Q_profile[mid_old_bin]) * Param_RE_Tterm_approx(Tterm, param_RA);
        }
        Earray[tbin] = Const * Integrate * skip_bins * atten_factor;
      }
    } // if EM_LQ > 0
    ...
    if ( event->Nu_Interaction[0].HAD_LQ > 0 ) {
      ...
      for (int tbin=0; tbin<outbin; tbin++) {
        ...
        for (int bin=0; bin<new_shower_bin-2; bin++) {
          ...
          if ( event->Nu_Interaction[0].HAD_shower_Q_profile[mid_old_bin]<=0) Integrate += 0.;
          else Integrate += -1.*(event->Nu_Interaction[0].HAD_shower_Q_profile[mid_old_bin]) * Param_RE_Tterm_approx(Tterm, param_RA);
        }
...
void Signal::GetVm_FarField_Tarray( Event *event, Settings *settings1, double viewangle, double atten_factor, int outbin, double *Tarray, double *Earray, int &skip_bins ) {
  ...
  if(settings1->SIMULATION_MODE==1){
    //use Eugene Hong's custom time domain model
    //best documentation is found in his thesis: http://rave.ohiolink.edu/etdc/view?acc_num=osu1417624919
    EugeneModule(event, settings1, viewangle, atten_factor, outbin, Tarray, Earray, skip_bins);
  }
  else if (settings1->SIMULATION_MODE==2){
    //we use the Hanson-Connolly model (https://arxiv.org/abs/1605.04975)
    //code base lives here: https://github.com/918particle/AskaryanModule
    //good tutorial here: https://github.com/918particle/AskaryanModule/tree/simplified/tutorial
    AskaryanModule(event, settings1, viewangle, atten_factor, outbin, Tarray, Earray, skip_bins);
  }
}
