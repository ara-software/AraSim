...
\begin{itemize}
 ...
 \item \textbf{Pick near} (mode 1, default): Chooses $\nu$ interaction position close by to the detector, within a cylinder of radius R. 
 
 \underline{In pick near mode only}:
...
\pr{SECONDARIES}=0 (default): does not allow any more interactions after the first. mode 1: allow secondary interactions (for future release only - do not use).

\pr{TAUDECAY}=0 (default) do not allow secondary $\tau$ particle decays. mode 1: let $\tau$ particles that are generated from $\nu_\tau$ to also have a secondary decay in the ice. Only works when \p{SECONDARIES} is set to 1 (future release). 
...
% \begin{itemize}
 ...
 \pr{BH ANT SEP DIST ON}=0 (default) the distances between antennas on a string are all the same:
 
 \pr{z btw}=10 (in meters) is the distance between antennas in mode 0.
 
 In mode 1 we get to choose different distances on the string. 
 
 \pr{z btw01}=2 (in meters) is the distance between the first (lowest) antenna and the second. 
 
 \pr{z btw12}=15 (in meters) between the second and third antenna. 
 
 \pr{z btw23}=2 (in meters) between the third and fourth (last, if it exists) antenna. 
...
\r{SECONDARIES}{0: no secondary interactions. 1: future release.}

\r{TAUDECAY}{0: do not allow secondary tau decay. 1: allow secondary decay of tau's, only works when \p{SECONDARIES}=1 in future release.}
...
\r[0]{TRIG SCAN MODE}{triggering algorithm. 0: old code. 1: new, faster code. 2: scan all powerthreshold values.}
...
\r{BH ANT SEP DIST ON}{0: all distances are the same (choose \p{z btw}). 1: choose each distance youself (use \p{z btwAB}).}
