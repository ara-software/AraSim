...
For selecting a single energy choose values \p{EXPONENT}= $N$; {\bf N is an integer}. When $10<N<20$, \arasim will generate neutrinos with energies $E=10^N$eV. Taking values too low or too high may lead to strange results in the simulation (The ARA detector should work well for neutrinos at energies of $E\sim 10^{15} - 10^{20}$eV only). 
...
\room
 ...
 \item \textbf{Pick exact} (mode 2): this chooses the position of all $\nu$'s to be at the same location. 

The position of interaction is built into \arasim at this time (look inside `Primaries.cc' for \p{INTERACTION MODE}==2). 
 
 The default is at $R=1000$m, $\th=-\pi/4 $ and $\phi=-\pi/6 $, which translates into 
  \begin{equation*}  
  x=353.55 \text{m} \quad y=612.37\text{m} \quad z=707.1\text{m}
   ...
   relative to the station center. This mode is useful for testing reconstruction at a specific location, or for making sure all events have a ray trace solution (see subsection \ref{ray solving}).
    ...
    \pr{POSNU\_THETA} interaction location elevation angle in radians

    \pr{POSNU\_PHI} interaction location azimuthal angle in radians

    \pr{POSNU\_R} interaction location radius in meters
...
\item \textbf{Above Ice} (mode 4): Throws events in a cylinder above the ice. \p{PICK\_ABOVE\_HEIGHT}=3000m (default) sets the height of the cylinder in which events will be thrown. Meant to be used with the arbitrary event generator mode \p{EVENT\_TYPE}=10 (see section on ``other parameters'').
...
\pr{RAY\_TRACE\_ICE\_MODEL\_PARAMS}=0 (default). Use a index of refraction model fitted from the RICE data (\url{https://doi.org/10.3189/172756504781829800}). =1 is Soth Pole fitted from RICE 2. =2 is South Pole measurement by Eisen 2003. =3 is South Pole measurement by Gow. =10 and 11 are both models for Moore's Bay. =20 is measurements at Byrd station by Ebimuna 1983. =30 is measurement at Mizuho station by Ebimuna 1983.
...
\begin{itemize}
 \item \p{DETECTOR} mode 0 unused. 
  ...
  \item \p{DETECTOR} mode 4: import real station geometry from AraRoot (requires an AraRoot installation. Make sure to choose a \p{DETECTOR\_STATION}.

  \pr{DETECTOR\_STATION}. Must be used with \p{DETECTOR}=4. Set equal to the station who's geometry you want to import (e.g.\p{DETECTOR\_STATION}=2, =3, etc). Includes the channel mapping to the AraRoot like data structure UsefulAtriStationEvent.
...
\pr{DATA LIKE OUTPUT}=0. This parameter determines what information is saved to `eventTree', which is supposed to represent the data which will be available for analysis in a real detector. It contains none of the information \arasim uses internally, only what is measured by the detector. The possible modes are:

\begin{itemize}

  \item mode 0 (default): no data like output.

  \item mode 1 : Output triggered events only.This is the most realistic case in which we only store waveforms for interesting events.

  \item mode 2: Output all events (signal only inserted into data-like output). This saves empty waveforms for non-globally triggered events. Useful for matching the events in `AraTree2' with those in 'eventTree' but will generate large outputs. 
...
\pr{WAVEFORM\_LENGTH}=640 (default). This sets the number of samples desired in the waveform output.

\pr{WAVEFORM\_CENTER}. This sets the number of bins offset from the current center at which you wish to examine the output. Default is the trigger bin. Basically how displaced is the trigger window from the triggering bin.

\pr{V MIMIC MODE}=0 (deprecated! Testbed only!). Every globally triggered event that is saved has a waveform closed off by a certain time window. The time to start recording and to end recording depends on the timing of the trigger and the time delays between channels. How much edges around the peak of the waveform will be saved into the file will depend on the offsets between channels, including factors such as station geometry and cable delays. 
...
\caption{An example of different time windows for modes of V\_MIMIC\_MODE  } %\p{V MIMIC MODE}
...
\pr{EVENT TYPE}=0 (default) for neutrinos. =10 arbitary event; this mode takes in a waveform file (currently ``arbitrary\_waveform.txt'') as if it was the E-field one meter from the antenna, and adds time delays.
...
\begin{itemize}
 \item \url{http://ara.physics.wisc.edu/cgi-bin/DocDB/ShowDocument?docid=1533}
\end{itemize}
...
\r[]{POSNU\_THETA}{Neutrino interaction location elevation angle in radians. For use with mode \p{INTERACTION\_MODE}=2.}

\r[]{POSNU\_PHI}{Neutrino  interaction location azimuthal angle in radians. For use with mode \p{INTERACTION\_MODE}=2.}

\r[]{POSNU\_R}{Neutrino interaction location radius in meters. For use with mode \p{INTERACTION\_MODE}=2.}
...
\r[0]{RAY TRACE ICE MODEL PARAMS}{What index of refraction model to use. See Settings.h for list of all available.}
...
\r[1]{DETECTOR}{0: unused. 1: ideal ARA station. can choose 1 to 7 stations. 2: large array in hexagonal design. 3: testbed. 4: Import geometry from AraRoot.}

\r[]{DETECTOR\_STATION}{2, 3, etc. What station's geometry to import from AraRoot. Only works with \p{DETECTOR}=4}
...
\r[0]{DATA LIKE OUTPUT}{ 0: no data-like output. 1: output triggered events only. 2: output all events (signal only inserted into data-like output).}
...
\r[640]{WAVEFORM LENGTH}{Set the number of samples desired in the waveform output}

\r[]{WAVEFORM CENTER}{The number of bins offset from the current center at which you wish to examine the output. Default center is the triggering bin.}
...
\r[0]{EVENT TYPE}{0: neutrinos. =10 is arbitrary event waveform located in ``arbitrary\_waveform.txt'' file.}
