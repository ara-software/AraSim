  ...
  for (int i = 0; i < detector->params.number_of_stations; i++) {
	...
	if (event->Nu_Interaction[0].pickposnu
	  ...
	  if (raysolver->solution_toggle) { // if there are solution from raysolver
		...
		else if (settings1->SIMULATION_MODE == 1 || settings1->SIMULATION_MODE==2) {

		  // if event is not calpulser
		  if (event->IsCalpulser == 0) {

		    if (settings1->EVENT_TYPE == 0) {//If not arbitrary events
      ...
      // std::cout << __FILE__ << " " << __FUNCTION__ << " " << __LINE__ << std::endl;
          ...
          stations[i].strings[j].antennas[k].V[ray_sol_cnt].push_back(
			...
			stations[i].strings[j].antennas[k].PeakV.push_back(
		      ...
		      /******   After this, arbitrary event type starts     ******/
	    ...
	    */
		...
		while(check_chan < ch_ID){
		  ...
		  if(string_i==0 && antenna_i==0){
		    ...
		    if(settings1->PHASING_MODE==1){									    high_check_val = phased_array_hi*sqrt(8);
		      lo_check_val = phased_array_lo*sqrt(8);
		      should_i_check_this_chan_for_trigger=true;
		    }
									  
		    else if(settings1->PHASING_MODE==0){
		  ...
		  */
		  ...
		  //dipoles + LPDAs must trigger on hi AND low
		  if(has_hi && has_lo){
		    channel_trig_this_hilo_bin[check_chan]=true; //mark this channel as being hit
		    // printf("		Hi and Lo Trigger (LPDA/Phased)!! Chan %d <-------------------\n");
		    //this way, if once you've shifted the 5ns over and this channel triggers again, it just keeps getting set hi
		  }
		  //the phased array may trigger on a hi OR low
		  if(string_i==0 && antenna_i==0 && (has_hi || has_lo) ){
		    channel_trig_this_hilo_bin[check_chan]=true; //mark this channel as being hit
		    // printf("		Hi or Lo Trigger (PA)!! Chan %d <-------------------\n");
		  }
		  check_chan++;
		}//loop over channels
...
double Report::GaintoHeight(double gain, double freq, double n_medium, int antType, int antMode) {
...
double Report::GaintoHeight(double gain, double zr, double freq, double n_medium, int antType, int antMode) {
  ...
  if (antType == 1 && antMode == 2)
    return sqrt(gain / 4 / PI * CLIGHT * CLIGHT / (freq * freq) * zr / Z0);
  /*
